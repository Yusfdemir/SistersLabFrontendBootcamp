const pokemons = [
  ...
  const filteredWater=pokemons.filter(pokemon=>pokemon.type==="Water")
  console.log(`"Water" türündeki Pokemon sayısı:${filteredWater.length}  `);
  console.log(`"Water" türündeki Pokemon isimleri:`);
  filteredWater.forEach(pokemon=>{console.log(pokemon.name)})
  ...
  const doubledExperience=pokemons.map(pokemon=>{
    const newPokemon={name:pokemon.name,experience:pokemon.experience*=2}
    return newPokemon
})
  ...
  const allWaterPokemonsHighExperience=filteredWater.every(pokemon=>pokemon.experience>60)
  console.log(
  ...
  const typeExperienceAverages=pokemons.reduce((total,pokemon)=>{return total+=pokemon.experience},0)/pokemons.length
